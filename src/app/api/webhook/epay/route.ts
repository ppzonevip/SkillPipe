import { NextRequest, NextResponse } from "next/server";
import { prisma } from "@/lib/prisma";
import crypto from "crypto";

function verifyEpaySignature(params: Record<string, string>, secret: string): boolean {
  const { sign, ...rest } = params;
  if (!sign) return false;

  // Sort parameters alphabetically
  const sortedKeys = Object.keys(rest).sort();
  const signStr = sortedKeys
    .filter((k) => rest[k])
    .map((k) => `${k}=${rest[k]}`)
    .join("&");

  const expectedSign = crypto
    .createHash("md5")
    .update(signStr + secret)
    .digest("hex");

  return sign === expectedSign;
}

export async function POST(req: NextRequest) {
  try {
    const body = await req.json();
    const { trade_no, out_trade_no, status, money } = body;

    if (!out_trade_no || !status) {
      return NextResponse.json({ error: "Missing required fields" }, { status: 400 });
    }

    // Get Epay config
    const secretConfig = await prisma.systemConfig.findUnique({
      where: { key: "epay_secret" },
    });

    // Verify signature
    if (secretConfig?.value && !verifyEpaySignature(body, secretConfig.value)) {
      return NextResponse.json({ error: "Invalid signature" }, { status: 400 });
    }

    // Only process successful payments
    if (status !== "success") {
      return NextResponse.json({ success: true });
    }

    // Find and update order
    const order = await prisma.order.findUnique({
      where: { orderNo: out_trade_no },
      include: { user: true },
    });

    if (!order) {
      return NextResponse.json({ error: "Order not found" }, { status: 404 });
    }

    if (order.status === "PAID") {
      return NextResponse.json({ success: true });
    }

    // Calculate membership expiration
    let daysToAdd = 30;
    if (order.period === "yearly") {
      daysToAdd = 365;
    }

    const now = new Date();
    let newExpireAt: Date;

    if (order.user.membershipExpireAt && order.user.membershipExpireAt > now) {
      newExpireAt = new Date(order.user.membershipExpireAt);
      newExpireAt.setDate(newExpireAt.getDate() + daysToAdd);
    } else {
      newExpireAt = new Date();
      newExpireAt.setDate(newExpireAt.getDate() + daysToAdd);
    }

    // Update order and user
    await prisma.$transaction([
      prisma.order.update({
        where: { id: order.id },
        data: {
          status: "PAID",
          paidAt: new Date(),
        },
      }),
      prisma.user.update({
        where: { id: order.userId },
        data: {
          membershipTier: order.tier,
          membershipExpireAt: newExpireAt,
        },
      }),
    ]);

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error("Epay webhook error:", error);
    return NextResponse.json({ error: "Webhook processing failed" }, { status: 500 });
  }
}
